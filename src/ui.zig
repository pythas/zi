...
pub const Ui = struct {
    ...
    pub fn button(self: *Self, rect: Rectangle, text: [:0]const u8) UiState {
        var state = UiState{};
        ...
        var color = self.style.button_bg_color;
        if (state.is_down) {
            color = self.style.button_bg_color_active;
        } else if (state.is_hovered) {
            color = self.style.button_bg_color_hovered;
        }
        ...
        self.label(.{ text_x, text_y }, text, font_size, self.style.text_color);
